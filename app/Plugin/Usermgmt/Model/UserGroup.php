...
class UserGroup extends UserMgmtAppModel {
	 ...
	 * @return array
	 */
	public function getGroups($single='') {
		$this->unbindModel(array('hasMany' => array('UserGroupPermission')));
		if($single==''){
			$result=$this->find("all", array("order"=>"id", "conditions"=>array('name !='=>"Guest",'name !='=>"Admin")));
		}
		else{
			$result=$this->find("all", array("order"=>"id", "conditions"=>array('id'=>$single)));
		}
		$user_groups=array();
